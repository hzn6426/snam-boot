...
docker images | grep snam/authority | grep -v grep | awk '{print $3}'| tail -n +3 | xargs $dockerbin rmi -f
...
mkdir -p /usr/local/snam/authority/logs
cd /usr/local/snam/authority || exit
$dockerbin build -t snam/authority:v"${md}" .
# shellcheck disable=SC2126
...
$dockerbin run -di --restart=always  --network=host --memory=4g --memory-swap=4g -v /usr/local/snam/authority/logs:/deployments/logs/snam-authority --name snam-authority"${md}" -p 8091:8091 snam/authority:v"${md}"
