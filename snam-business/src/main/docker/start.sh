...
docker images | grep snapper/business | grep -v grep | awk '{print $3}'| tail -n +3 | xargs $dockerbin rmi -f
...
cd /usr/local/snapper/business || exit
$dockerbin build -t snapper/business:v"${md}" .
# shellcheck disable=SC2126
...
$dockerbin run -di --restart=always --network=host --name snam-business"${md}" -p 8092:8092 snapper/business:v"${md}"
